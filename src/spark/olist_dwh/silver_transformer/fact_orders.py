...
def process_dim_date_df(spark: SparkSession, dim_date_df: DataFrame, orders_df: DataFrame, order_items_df: DataFrame) -> DataFrame:
    ...
    return spark.sql(
        """
            SELECT  o.order_id AS d_order_id
                    ,d1.date_id AS shipment_limit_date_id
            ...
            FROM orders AS o
            JOIN order_items AS oit ON oit.order_id = o.order_id
            LEFT JOIN dim_date AS d1 ON d1.unix_timestamp = UNIX_TIMESTAMP(oit.shipment_limit_date)
...
def create_fact_orders_df(
    ...
    processed_dim_date_df = process_dim_date_df(spark=spark, dim_date_df=dim_date_df, orders_df=orders_df, order_items_df=order_items_df)
    ...
    fact_orders_df = orders_df.join(order_items_df, on=F.col("order_id") == F.col("order_item_order_id"), how="inner") \
        ...
        .join(processed_dim_date_df, on=F.col("d_order_id") == F.unix_timestamp("order_id"), how="inner") \
        .select(
            "order_id",
            "order_item_id",
            "product_id",
