...
def create_dim_date_df(date_df: DataFrame) -> DataFrame:
    ...
    dim_date_df = (
        date_df.withColumn("date_id", F.row_number().over(Window.orderBy("timestamp")))
        ...
        .withColumn(
            "time_of_day",
            F.when(F.date_format(F.col("timestamp"), "HH:mm").between("00:01", "05:00"), "Dawn")
            .when(F.date_format(F.col("timestamp"), "HH:mm").between("05:01", "11:59"), "Morning")
            .when(F.date_format(F.col("timestamp"), "HH:mm").between("12:00", "18:00"), "Afternoon")
            .when(F.date_format(F.col("timestamp"), "HH:mm").between("18:01", "24:00"), "Night"),
        )
    )
    return dim_date_df
